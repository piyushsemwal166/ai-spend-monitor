...
export default function TeamBudgetsPage() {
  const [budgets, setBudgets] = useState<TeamBudgetRow[]>([]);
  ...
  useEffect(() => {
    ...
    const loadBudgets = async () => {
      try {
        const response = await teamBudgetService.getTeamBudgets({ page: meta.page, limit: meta.limit, search: search || undefined, sortBy: "updatedAt", sortOrder: "desc" });
  ...
  const deleteMutation = useMutation({
    mutationFn: (budgetId: string) => teamBudgetService.deleteTeamBudget(budgetId),
    onSuccess: async () => {
      if (user) {
        const response = await teamBudgetService.getTeamBudgets({ page: meta.page, limit: meta.limit, search: search || undefined, sortBy: "updatedAt", sortOrder: "desc" });
        setBudgets(response.items);
  ...
  const handleDelete = async (budgetId: string, teamName: string) => {
    if (!window.confirm(`Delete the budget for ${teamName}?`)) {
      return;
    }
  ...
  if (isAuthLoading || isLoading) {
    return <p className="px-5 py-6 text-sm text-slate-500 dark:text-slate-400">Loading team budgets...</p>;
  }

  if (!user) {
    return <EmptyState title="Sign in to view team budgets" description="Team budget management is hidden until you authenticate." actionLabel="Go to login" />;
  }
  ...
  return (
    <div className="space-y-8 pb-10">
      <PageHeader
        eyebrow="Budgets"
        title="Team budgets"
        description="Track spend limits at the team level and surface overruns before they grow."
        actions={
          <>
            ...
            <Link href="/team-budgets/create" className={buttonClassName({ variant: "primary" })}>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex w-full max-w-xl gap-3">
          <Input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search team budgets" />
      ...
      <Card className="border-white/10 bg-white/75 dark:bg-slate-950/55">
        <CardHeader>
          <CardTitle>Team budget list</CardTitle>
        </CardHeader>
        <CardContent>
          <DataTable<TeamBudgetRow>
            rowKey={(row) => row.id}
            columns={[
              { header: "Team", cell: (row) => row.team?.name ?? row.teamId },
              { header: "Organization", cell: (row) => row.team?.organization?.name ?? "-" },
              ...
              { header: "Alert threshold", cell: (row) => `${row.alertThreshold}%` },
              {
                ...
                cell: (row) => (
                  <div className="flex items-center gap-3">
                    <Link href={`/team-budgets/${row.id}/edit`} className="font-medium text-cyan-600 hover:text-cyan-500 dark:text-cyan-300">
                      Open
                    </Link>
                    <button
                      type="button"
                      onClick={() => void handleDelete(row.id, row.team?.name ?? row.teamId)}
